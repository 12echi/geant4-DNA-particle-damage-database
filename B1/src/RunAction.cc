...
RunAction::RunAction()
  ...
  accumulable_manager->RegisterAccumulable(let);
  accumulable_manager->RegisterAccumulable(CDSB);
...
void RunAction::EndOfRunAction(const G4Run *run) {
  ...
  G4int totalCDSB = CDSB.GetValue();
  G4int totallet = let.GetValue();
  ...
  if (nofEvents != 0) {
    G4double absorbed_dose = (total_energy_deposition / nofEvents / joule)
        / (G4LogicalVolumeStore::GetInstance()->GetVolume("Logical targer")->GetMass() / kg);
     G4double LET = (totallet / nofEvents);

    G4double ssb_yield = static_cast<double>(total_ssb_num) / nofEvents / absorbed_dose / 6;
    G4double cssbn_yield = static_cast<double>(total_dsb_num) / nofEvents / absorbed_dose / 6;
    ...
    if (IsMaster()) {
      ...
      G4cout << "| SSB/DSB | " << dsbn_yield/ssb_yield   << G4endl;
      ...
      ofstream DataOutOPut;
              DataOutOPut.open("./data2/DataOutOPut.txt", ios::app);
              DataOutOPut << "Energy:\t" << Energy << "\t";
              //DataOutOPut << "ssb_num:\t" << static_cast<double>(total_ssb_num) / nofEvents << "\t";
              //DataOutOPut << "cssbn_num:\t" << static_cast<double>(total_cssbn_num) / nofEvents  << "\t";
              DataOutOPut << "dsbn_num:\t" << static_cast<double>(total_dsb_num) / nofEvents << "\t";
              DataOutOPut << "energy_deposit:\t" << total_energy_deposition / nofEvents / keV << " keV"<< "\t";
              DataOutOPut << "cdsbn_num:\t" << static_cast<double>(totalCDSB) / nofEvents <<"\t";
              //DataOutOPut << "ssb_yield:\t" << ssb_yield << "\t";
              //DataOutOPut << "dsb_yield:\t" << dsbn_yield << "\t";
